...
export function normalizeLandmarks(landmarks) {
  ...
  // Primary: mid-hip to mid-shoulder
  if (lHip && rHip && lSho && rSho && (lHip.visibility ?? 0) > 0.3 && (rHip.visibility ?? 0) > 0.3) {
    origin = {
      x: (lHip.x + rHip.x) / 2,
      y: (lHip.y + rHip.y) / 2,
  ...
  } else if (lSho && rSho && (lSho.visibility ?? 0) > 0.3 && (rSho.visibility ?? 0) > 0.3) {
    // Secondary fallback: shoulder midpoint and shoulder width as reference scale
    origin = {
      x: (lSho.x + rSho.x) / 2,
      y: (lSho.y + rSho.y) / 2 + 0.3, // approximate hip position below shoulders
      z: ((lSho.z ?? 0) + (rSho.z ?? 0)) / 2,
    };
    const shoWidth = Math.sqrt(
      (lSho.x - rSho.x) ** 2 +
      (lSho.y - rSho.y) ** 2 +
      ((lSho.z ?? 0) - (rSho.z ?? 0)) ** 2
    ) || 0.3;
    torsoLen = shoWidth * 1.5;
  } else {
    // Fallback center
...
import { POSE_DEFINITIONS, mapPoseNameToKey } from "./pose_definitions.js";
 ...
 *
 * @param {Object|string} poseDefinition - POSE_DEFINITIONS[poseKey] or string pose key/name
 * @param {Object} angles                - { jointName: { angle, visible } } (EMA-smoothed)
 * @returns {Object} Evaluation result
 */
export function evaluatePose(poseDefinition, angles = {}) {
  let def = poseDefinition;
  if (typeof def === "string") {
    const k = mapPoseNameToKey(def);
    def = POSE_DEFINITIONS[k] || POSE_DEFINITIONS.cobra;
  } else if (!def || !def.joints) {
    def = POSE_DEFINITIONS.cobra;
  }
  ...
  return {
    pose:     def.description || def.name || "Yoga Pose",
    score,
    correct:  poseCorrect,
    joints:   jointResults,
