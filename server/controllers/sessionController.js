...
export const startSession = asyncHandler(async (req, res) => {
  ...
  // Get patient's preferred language and signup consent
  const patient = await Patient.findById(patientId);
  const requestedLang = language || req.headers["x-patient-language"];
  const lang = requestedLang || patient?.preferredLanguage || "en";
  ...
  const session = await Session.create({
    ...
    consent: {
      dataCollectionConsent: patient?.consent?.dataCollection ?? true,
      dataSharingConsent: patient?.consent?.dataSharing ?? true,
      abhaLinkingConsent: true,
      consentTimestamp: patient?.consent?.consentDate || new Date(),
      consentMethod: "touch",
    },
  });
  ...
  // Get first question in patient's selected language
  const langData = FIRST_QUESTIONS[lang] || FIRST_QUESTIONS.en || FIRST_QUESTIONS.hi;
  const firstQuestion = {
...
/**
 * POST /api/session/:sessionId/verify
 * Patient confirms the generated history, optionally with corrections
 * they made on the verification screen.
 */
export const verifySession = asyncHandler(async (req, res) => {
  ...
  if (session.patient.toString() !== req.userId) {
    throw new ApiError(403, "You don't have permission to verify this session");
  }

  const cleanCorrections = Array.isArray(corrections)
  ...
  session.patientVerification = {
    verifiedAt: new Date(),
    corrections: cleanCorrections,
  };

  // Surface the corrections in the doctor-facing summary too, so the
  // doctor sees them without needing to query this field separately.
  if (cleanCorrections.length > 0 && session.clinicalSummary?.generatedText) {
    const correctionLines = cleanCorrections
      .map(
        (c) =>
          `- ${c.section} / ${c.label}: "${c.original}" corrected to "${c.corrected}"`,
      )
      .join("\n");
    session.clinicalSummary.generatedText = `${session.clinicalSummary.generatedText}\n\nPATIENT-VERIFIED CORRECTIONS:\n${correctionLines}`;
  }
  ...
  res.status(200).json(
    new ApiResponse(
      200,
      {
        ...
        verifiedAt: session.patientVerification.verifiedAt,
        correctionCount: cleanCorrections.length,
      },
