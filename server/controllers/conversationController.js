...
import { appendInterviewContext } from "../services/medicalHistoryService.js";

/** Helper: extract interview context from a completed session and push into medical history */
async function _pushInterviewToMedHistory(session) {
  try {
    const ch = session.clinicalHistory || {};
    const qa = session.conversation
      .filter((m) => m.role === "patient")
      .map((m, i) => {
        const aiMsg = session.conversation.filter((x) => x.role === "ai")[i];
        return { question: aiMsg?.content || "", answer: m.content };
      });

    await appendInterviewContext(session.patient._id || session.patient, {
      sessionId: session._id,
      chiefComplaint: ch.chiefComplaint || "",
      symptoms: ch.hpiDetails?.associatedSymptoms || [],
      duration: ch.hpiDetails?.timing || "",
      severity: ch.hpiDetails?.severity || null,
      interviewQA: qa,
    });
  } catch (err) {
    console.error("[MedHistory] Failed to push interview context:", err.message);
  }
}
...
export const respondToQuestion = asyncHandler(async (req, res) => {
  const { sessionId, answer, inputMode, language } = req.body;
  ...
  // Get patient's preferred language (prioritizing request body / session)
  const patientLang =
    language ||
    req.headers["x-patient-language"] ||
    session.language ||
    session.patient?.preferredLanguage ||
    "en";
  ...
  // ── Push interview data into Medical History on completion ───────
  if (session.status === "completed") {
    await _pushInterviewToMedHistory(session);
  }
