...
export const generateSummary = asyncHandler(async (req, res) => {
  ...
  // Fetch full unified medical history for richer AI context
  let medicalHistoryContext = "";
  try {
    medicalHistoryContext = await getMedicalHistoryForAI(session.patient._id.toString());
  } catch (e) {
    console.error("[Summary] Could not fetch med history context:", e.message);
  }

  // Generate bilingual summary using AI (with full medical history context)
  const summaryData = await generateClinicalSummary(session, patientLang, medicalHistoryContext);
