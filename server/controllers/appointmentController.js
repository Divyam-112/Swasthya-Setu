...
export const bookAppointment = asyncHandler(async (req, res) => {
  ...
  // Optionally verify session if provided
  let session = null;
  if (sessionId) {
  ...
  if (session) {
    appointmentData.session = session._id;
  }

  const appointment = await Appointment.create(appointmentData);

  // Link the appointment to the session if present
  if (session) {
...
export const getDoctorQueue = asyncHandler(async (req, res) => {
  ...
  // Match current doctor or any doctor account sharing the same doctor name
  let doctorIds = [doctorId];
  const currentDoc = await Doctor.findById(doctorId);
  if (currentDoc?.name) {
  ...
  // If no appointments scheduled specifically for today, check if this doctor has appointments from seed/demo
  if (appointments.length === 0 && !date) {
    const anyFilter = {
      doctor: { $in: doctorIds },
    };
    if (status) anyFilter.status = status;

    const anyAppointments = await Appointment.find(anyFilter)
      .populate("patient", "name age gender abhaId preferredLanguage phone")
      .populate({
        path: "session",
        select:
...
export const updateAppointmentStatus = asyncHandler(async (req, res) => {
  ...
  let doctorIds = [doctorId.toString()];
  const currentDoc = await Doctor.findById(doctorId);
  if (currentDoc?.name) {
    const cleanName = currentDoc.name.replace(/^Dr\.\s*/i, "").trim();
    if (cleanName.length >= 3) {
      ...
      doctorIds = matchingDocs.map((d) => d._id.toString());
    }
  }
